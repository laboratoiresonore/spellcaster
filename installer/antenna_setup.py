...
REM
REM  Hub:   {hub_url}
REM  Token: embedded below (rotate via `antenna rotate-token`)
REM ==========================================================
...
REM ── Fetch the antenna bundle from the hub on first launch ──
REM     (Hub exposes /api/antenna/bundle.zip — see hub README)
if not exist antenna\agent.py (
    echo Fetching antenna bundle from hub {hub_url} ...
    python -c "import urllib.request, zipfile, io; r=urllib.request.urlopen('{hub_url}/api/antenna/bundle.zip', timeout=30); zipfile.ZipFile(io.BytesIO(r.read())).extractall('.')" || goto :fetch_failed
)

REM ── Write config if missing (one-time bootstrap) ──
...
:fetch_failed
echo.
echo   Failed to reach the Spellcaster hub at {hub_url}.
echo   - Make sure the hub machine is on and running Spellcaster.
echo   - Check that the IP is reachable (try: ping {hub_host_ip})
echo.
pause
exit /b 2
...
if [ ! -f antenna/agent.py ]; then
    echo "Fetching antenna bundle from hub {hub_url} ..."
    python3 -c "import urllib.request, zipfile, io; r=urllib.request.urlopen('{hub_url}/api/antenna/bundle.zip', timeout=30); zipfile.ZipFile(io.BytesIO(r.read())).extractall('.')"
fi
