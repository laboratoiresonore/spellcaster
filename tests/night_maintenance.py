...
def main() -> int:
    ...
    ap.add_argument("--dry-mode", action="store_true",
                    help="Skip network-dependent checks (installer-audit, "
                         "capabilities). Still runs mirror-drift, cross-repo "
                         "drift, model-paths and log-scan. Used by CI on "
                         "GitHub-hosted runners that can't reach the LAN "
                         "ComfyUI/caps servers.")
    args = ap.parse_args()
    ...
    if args.dry_mode:
        ...
        # the dev-host Windows D: drive layout — neither is reachable from
        # a GitHub-hosted runner. Static checks (mirror-drift, cross-repo
        # drift, log-scan) still run and are what the CI preflight cares
        # about.
