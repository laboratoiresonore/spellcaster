...
class TestFetchMetadataScript(unittest.TestCase):
    ...
    def test_accessibility_note_contains_public(self):
        ...
        # network error we can't do anything about from a sandboxed / unauthenticated
        # runner (rate limits, 403 forbidden, or a generic HTTP error from the GitHub
        # anon API). The functional cases live in test_metadata_json_structure /
        # test_metadata_field_types; this test only guards the label wording.
        ...
        self.assertTrue(
            ...
            f"accessibility_note should contain 'public' or a known-transient error marker, got: {accessibility_note}"
        )
