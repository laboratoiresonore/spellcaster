...
| ❌ SKIP | `pynsist` | Voodoomancer FirstRun.bat orchestration | ~80 | MIT | **dormant — no release since 2022** |
...
## 2. `age` / `pyrage` — ADOPT NOW (Voodoomancer only)
...
- **Gotchas**:
  - mDNS is link-local. Multi-subnet ComfyUI setups (user's workstation on WiFi, GPU box on wired LAN) lose discovery that our HTTP broker gets for free via `X-Forwarded-For`. That's a real use case in Whimweaver.
...
- If Voodoomancer bundles stay Windows-only (current plan), briefcase is overkill for a week of setup.
- Decision trigger: when macOS + Linux Voodoomancer becomes a real release goal, revisit.
...
**Sprint 2 (weeks):**
4. `age` / `pyrage` for Voodoomancer — replace the `.scv` spec entirely.
