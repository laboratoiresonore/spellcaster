...
`.github/workflows/leak-check.yml` is already red against `main` (79 hits from the same forbidden-pattern list in files not authored by this PR — private-project code-names, an internal hostname/IP, and a maintainer email pattern — spread across a retired-service inventory doc, the krita plugin defaults, `installer/remote_services.json`, workflow comments, and several `spellcaster_core/` docstrings). This PR does not add any new hits (the mirror-drift fix only propagates content already present on surface C into surface 1). Scrubbing that pre-existing surface is out of scope for a research-digest PR — it would touch ~20 files across multiple subsystems — and remains a Tier-1 candidate for a dedicated future cleanup PR.
