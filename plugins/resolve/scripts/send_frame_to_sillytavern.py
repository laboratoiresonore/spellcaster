"""Send Frame → SillyTavern

Grabs the playhead frame and publishes a ``sillytavern.asset.send``
event. SillyTavern's /sc-inbox slash command pulls it and renders in
chat — useful when the editor wants a storyboard frame referenced in
a chat with a character or worldbook.

Menu: Workspace > Scripts > 💎 Spellcaster > 💎 send_frame_to_sillytavern
"""
...
def main() -> int:
    ...
    try:
        req = urllib.request.Request(
            ...
            data=_json.dumps({
                "origin": "resolve",
                "kind": "frame_grab",
                "title": "Frame from Resolve → SillyTavern",
                "tags": ["to_sillytavern", "resolve_export"],
                "body_b64": b64,
            }).encode("utf-8"),
        ...
        show_message("Spellcaster", f"Couldn't upload frame:\n{e}")
        return 1
    finally:
        try:
            os.unlink(png_path)
    ...
    show_message(
        "Spellcaster",
        f"Frame sent to SillyTavern.\n\n"
        f"Asset: {asset_hash[:10]}…\n\n"
        f"In SillyTavern type /sc-inbox to render it in the active chat.",
    )
    return 0
