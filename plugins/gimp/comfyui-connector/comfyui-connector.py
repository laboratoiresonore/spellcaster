...
def _build_faceid_img2img(target_filename, face_ref_filename, preset_key,
    ...
    return build_faceid_img2img(target_filename, face_ref_filename, preset,
                                ...
                                lora_strength=lora_strength, weight=weight,
                                weight_v2=weight_v2,
                                denoise=denoise, steps=steps, cfg=cfg,
                                loras=loras)
...
def _build_pulid_flux(target_filename, face_ref_filename,
    ...
    """→ Delegated to v2 builder."""
    loras = turbo_loras if turbo and turbo_loras else None
    return build_pulid_flux(target_filename, face_ref_filename,
                            prompt_text, negative_text, seed,
                            flux_model=flux_model, pulid_model=pulid_model,
                            strength=strength, steps=steps, guidance=guidance,
    ...
    def do_create_procedure(self, name):
        menu_map = {
            ...
            "spellcaster-body-factory": ("2. Body Double (Full Body Ref)...", self._run_body_factory,
                                          "Stunt casting — generate full-body reference with your actor's face"),
            "spellcaster-clothing-store": ("3. Wardrobe Department...", self._run_clothing_store,
            ...
            "spellcaster-wan-director": ("5. Director's Chair (Solo)...", self._run_wan_director,
...
"spellcaster-upscale": ("Upscale 4x...", self._run_upscale,
    ...
    def _run_klein_blend(self, procedure, run_mode, image, drawables, config, data):
        ...
        try:
            ...
            for run_i in range(runs):
                ...
                wf = build_klein_blend(
                    fg_name, bg_name, prompt, seed,
                    blend_mode=blend_mode, opacity=opacity, scale=scale,
    ...
    def _run_klein_repose(self, procedure, run_mode, image, drawables, config, data):
        ...
        try:
            ...
            for run_i in range(runs):
                ...
                wf = build_klein_repose(
                    uname, klein_key, prompt, seed,
                    steps=steps, denoise=denoise, guidance=1.0,
        ...
        try:
            ...
            if export_mp4:
                try:
                    ...
                    # Build workflow: LoadImage chain → ImageBatch → VHS_VideoCombine
                    out_fps = 1000.0 / max(1, delay)
                    wf = build_frame_assembly(
                        frame_names, fps=out_fps,
                        filename_prefix="gimp_gif_stitch",
                    )
    ...
    def _run_upscale(self, procedure, run_mode, image, drawables, config, data):
        ...
        try:
            ...
            results = _run_with_spinner("Upscale: processing on ComfyUI...",
                                        lambda: list(_run_comfyui_workflow(srv, wf, timeout=600)))
            for i, (fn, sf, ft) in enumerate(results):
                _apply_mask_mode(srv, image, _download_image(srv, fn, sf, ft), f"Upscale {preset_key} #{i+1}", False)
            Gimp.displays_flush()
            Gimp.progress_end()
            return procedure.new_return_values(Gimp.PDBStatusType.SUCCESS, GLib.Error())
        except Exception as e:
    ...
    def _run_settings(self, procedure, run_mode, image, drawables, config, data):
        ...
        bx = dlg.get_content_area()
        bx.set_spacing(10); bx.set_margin_start(16); bx.set_margin_end(16)
        bx.set_margin_top(16); bx.set_margin_bott