...
**Compat shims (legacy, do not extend):** `/api/cached_asset/<name>` still serves files from the pre-refactor flat cache so old `generated_assets.json` entries keep working. `_cache_comfyui_asset` falls back to that flat cache only when `_ASSET_GALLERY is None` at import (cross-interface backbone failed to initialize). New code paths must tolerate both URL shapes when parsing (`'/api/assets/' in url or '/api/cached_asset/' in url`), but must produce only the canonical shape. As of 2026-04-20 the bundled-defaults seeder (`_seed_bundled_assets` in `tavern/server.py`) also routes through `AssetGallery.put` — fresh installs no longer write `/api/cached_asset/` URLs into `generated_assets.json`.

**Alternative transport — ComfyUI blob bus (tier-3 audit, 2026-04-20):** Every plugin with a ComfyUI URL can push bytes to `POST /spellcaster/blob/put` (on the ComfyUI pack) and broadcast the returned absolute URL through the Guild event bus. The `AssetGallery` path above is still the CANONICAL persistent store; blob bus is a transport optimization for LAN setups where bytes don't need to hop through the Guild's machine. Use it via `CrossInterfaceClient.blob_put()` (Python) or a curl `-F` multipart POST (Lua/other). Blob TTL is 1h default (24h max), hard ceiling 256 MB/blob + 2 GB aggregate. Senders prefer blob bus and fall back to `AssetGallery` on failure; receivers are URL-shape-agnostic (both `/api/assets/<hash>` and `http://comfy:8188/spellcaster/blob/<hash>` are plain http). See §20 for the full backbone picture and routes.
