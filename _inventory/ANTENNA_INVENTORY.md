...
- `prometheus_link.py` (~180) — Prometheus fleet-frame heartbeat publisher (parallel to `heartbeat.py` which targets the Guild). POSTs to `{cfg.prometheus_url}/api/heartbeat` every 5 s with `hostname / tailnet_ip / antenna_version / antenna_port / bearer_token / services_detected`. No-op when `prometheus_url` is empty. Added 2026-06-13 alongside the Fleet Frame deploy on Prometheus :8100.
