...
def patch_klein_nsfw(dest_dir):
    ...
    if inpaint_presets:
        ...
        for label, data in inpaint_presets.items():
            ...
            if "lora" in data:
                lora_line = (
                    f"                \"lora\": {{\"path\": {repr(data['lora']['path'])}, "
                    f"\"strength\": {data['lora']['strength']}}},\n"
                )
            nsfw_block += (
                f"            {repr(label)}: {{\n"
    ...
    # ── 3b. Inject NSFW clothing presets into Body Factory ────────────
    # The CLOTHING_STORE dict in the Body Factory / Re-dresser dialog
    # uses "label": "prompt text". Inject NSFW outfits after "Costume — superhero".
    clothing_anchor = '"Costume \u2014 superhero":'
    if clothing_anchor in content:
        nsfw_clothing = (
            '\n'
        ...
        idx = content.index(clothing_anchor)
        nl = content.index("\n", idx)
        content = content[:nl + 1] + nsfw_clothing + content[nl + 1:]
        injected += 6
        print(f"  Injected 6 NSFW outfits into Body Factory Clothing Store")
...
def patch_auth_auto_update(dest_dir):
    ...
    # Wizard Guild launcher: redirect auto-update to NSFW repo + inject auth
    guild_launcher = dest_dir / "tavern" / "guild_launcher.py"
    if guild_launcher.exists():
        content = guild_launcher.read_text(encoding="utf-8")

        # Redirect repo URL
        content = content.replace(
            '_GUILD_REPO = "laboratoiresonore/spellcaster"',
            f'_GUILD_REPO = "{NSFW_REPO}"'
        )
        ...
        if token:
            content = content.replace(
                ...
                f'_GUILD_AUTH_TOKEN = "{token}"  # NSFW: PAT for private repo access'
            )
            print(f"  Wizard Guild auto-update: redirected to {NSFW_REPO} + auth token injected")
        else:
            print(f"  Wizard Guild auto-update: redirected to {NSFW_REPO} (NO AUTH)")

        guild_launcher.write_text(content, encoding="utf-8")
...
# ─── Patch: Wizard Guild NSFW avatar + background asset generation ────────

def patch_guild_nsfw(dest_dir):
    """Inject NSFW appearance pools, prompt styles, and background prompts
    into the Wizard Guild server.py.

    The SFW server.py has empty anchor lists:
    ...
    This patch replaces those empty lists with provocative NSFW content so
    that when NSFW_MODE is True, the guild generates seductive wizard
    avatars and risqué tavern backgrounds during first-run asset generation.
    """
    server_path = dest_dir / "tavern" / "server.py"
    if not server_path.exists():
        print("  WARNING: tavern/server.py not found, skipping Guild NSFW patch")
        return

    content = server_path.read_text(encoding="utf-8")
    patched = 0

    # ── 1. NSFW Appearance Core Pool (14 entries — collision-free for 6 studios) ──
    old_core = "_NSFW_APPEARANCE_CORE = []"
    new_core = '''_NSFW_APPEARANCE_CORE = [
    # women (4)
    ...
    # ── 2. NSFW Appearance Discovered Pool (34 entries) ──
    old_disc = "_NSFW_APPEARANCE_DISCOVERED = []"
    new_disc = '''_NSFW_APPEARANCE_DISCOVERED = [
    # ── Women ──
    ...
    # ── 3. NSFW Prompt Styles ──
    old_styles = "_NSFW_PROMPT_STYLES = []"
    new_styles = '''_NSFW_PROMPT_STYLES = [
    ...
    # ── 4. NSFW Background Prompts (for batch generation) ──
    old_bg = "_NSFW_BG_PROMPTS = []"
    new_bg = '''_NSFW_BG_PROMPTS = [
    ...
    # ── 5. NSFW Background Styles (for the /api/background_generate endpoint) ──
    old_bg_styles = "BG_STYLES_NSFW = {}"
    new_bg_styles = '''BG_STYLES_NSFW = {
    ...
    if old_bg_styles in content:
        content = content.replace(old_bg_styles, new_bg_styles)
        patched += 1

    # ── 6. NSFW Wizard Persona (system prompt personality overlay) ──
    old_persona = '_NSFW_WIZARD_PERSONA = ""'
    new_persona = '''_NSFW_WIZARD_PERSONA = (
    ...
    # ── 7. NSFW Name Generation Prompt ──
    old_namegen = '_NSFW_NAME_GEN_PROMPT = ""'
    new_namegen = '''_NSFW_NAME_GEN_PROMPT = (
    ...
    if old_namegen in content:
        content = content.replace(old_namegen, new_namegen)
        patched += 1

    # ── 8. NSFW Meta System Addendum (injected into /api/system_prompt) ──
    old_meta = '_NSFW_META_SYSTEM_ADDENDUM = ""'
    new_meta = '''_NSFW_META_SYSTEM_ADDENDUM = (
    "NSFW CONTENT RULES:\\n"
    ...
    # ── 9. NSFW Arch Profiles (seductive archetypes for comfyui_model wizards) ──
    old_arch = '_NSFW_ARCH_PROFILES = {}'
    new_arch = '''_NSFW_ARCH_PROFILES = {
    "flux2klein": {
    ...
    # ── 10. Cinematic wizard: inject NSFW director scripts ──
    cinematic_anchor = 'IMPORTANT: Each step\'s image_filename will be auto-filled by the system'
    if cinematic_anchor in content:
        nsfw_scripts = (
            "\\n\\n"
        ...
        content = content.replace(cinematic_anchor, cinematic_anchor + nsfw_scripts, 1)
        patched += 1
        print("  Injected NSFW director scripts into Cinematic wizard")

    # ── 11. Studiocraft wizard: inject NSFW studio hints ──
    studiocraft_anchor = "- If user says 'just make a video' -> delegate to Videomancer"
    if studiocraft_anchor in content:
        nsfw_studio = (
            "\\n\\n"
        ...
        content = content.replace(studiocraft_anchor, studiocraft_anchor + nsfw_studio, 1)
        patched += 1
        print("  Injected NSFW studio hints into Studiocraft wizard")

    server_path.write_text(content, encoding="utf-8")
    print(f"  Patched Wizard Guild NSFW: {patched}/11 injection points")

    # ── 12. Patch guild_launcher.py to pass NSFW_MODE ──
    # (Already wired via _GUILD_AUTH_TOKEN detection — no patch needed,
    #  the auth token injection from patch_auth_auto_update handles this)


def get_nsfw_runtime_content():
    """Return all NSFW content as a dict for runtime injection.

    Called by guild_launcher.py when it detects nsfw/.github_token at startup
    (running from source, not a patched build). This lets the server module
    receive NSFW content without file patching.
    """
    return {
        "_NSFW_APPEARANCE_CORE": [
            "a stunning East Asian sorceress with flowing raven hair barely concealed by translucent silk robes that shimmer with arcane sigils, confident smouldering gaze",
        ...
        "_NSFW_APPEARANCE_DISCOVERED": [
            ...
            "a nine-tailed fox spirit in humanoid form, lithe naked body wreathed in flame-tipped tails that strategically conceal and reveal, feral seduction",
            "a living suit of ornate armour with no occupant, visor glowing blue, chest plate cracked open to reveal swirling naked energy within, intimate and alien",
            "a mushroom sage with spotted cap, bioluminescent gills casting warm light on surprisingly sensual humanoid curves beneath",
            "a clockwork courtesan-automaton with brass gears and copper patina, elegant mechanical form with strategic gaps showing warm light within, single emerald eye",
            "a moth-winged seductress with powdery iridescent wings and enormous compound eyes, nude humanoid torso dusted with shimmering scales, otherworldly allure",
            "an ent-like dryad spirit with bark skin artfully covering only the essentials, moss trailing like lingerie, firefly swarm illuminating smooth wooden curves",
            "a luminous jellyfish-being with translucent tentacles, humanoid core visible through ethereal membrane, sensual and alien in equal measure",
        ...
        "_NSFW_PROMPT_STYLES": [
            ("intimate extreme close-up portrait",
             "sultry bedroom lighting, magical aura caressing bare skin, highly detailed face and décolletage filling the frame, half-lidded seductive eyes, sensual parted lips, painterly digital art, headshot composition, dark moody background with arcane sigils, face takes up 80 percent of image"),
            ("provocative cinematic portrait, medium close-up",
             "volumetric warm light, magical particles dancing across exposed skin, shallow depth of field, revealing costume with intricate details visible, warm golden skin tones, renaissance nude painting meets fantasy concept art, dramatic chiaroscuro, sensual atmosphere, bedroom eyes"),
            ("dramatic half-body portrait from below, seductive power pose",
             "towering perspective, sheer robes billowing, arcane energy crackling across bare skin, rich fabric textures barely concealing, ornate magical body jewellery, deep moody atmosphere, cinematic lighting, concept art style, strong sensual silhouette, provocative stance"),
            ("intimate profile portrait in candlelight, post-coital glow",
             "single warm candle flame, rim lighting on bare shoulder, glistening skin, visible breath in cool air, intricate body art and enchanted piercings catching the light, old masters nude study style, rich shadows, intimate boudoir atmosphere, magical sigils floating lazily in afterglow"),
        ],
        "_NSFW_BG_PROMPTS": [
            "interior of a decadent magical pleasure guild, silk curtains and velvet chaises, warm amber candlelight, scattered enchanted wine goblets, arcane love potions on shelves, intimate alcoves with sheer draping, rose petals floating in enchanted air, wide angle, fantasy boudoir concept art, high quality",
            "opulent wizard bathhouse interior, steaming enchanted pools with glowing runes beneath the water, marble columns draped in translucent silk, magical incense smoke curling through warm light, scattered robes on heated stone, wide angle, sensual fantasy atmosphere, concept art",
        ...
        "_NSFW_ARCH_PROFILES": {
            "flux2klein": {"archetype": "a ravishing sorcerer channelling raw flux energy, prismatic fractals caressing bare skin, power and seduction intertwined", "scaffold": "studio_imaginus", "subtext_hint": "Flux 2 Klein — Explicit Image Generation"},
            "flux1dev": {"archetype": "a smouldering conjurer of photorealistic fantasies, light bending around glistening skin, every detail rendered in sinful clarity", "scaffold": "studio_imaginus", "subtext_hint": "Flux — Photorealistic NSFW Generation"},
            "sdxl": {"archetype": "a sensual artist-mage painting forbidden worlds with broad strokes of desire, canvases blushing at their own content", "scaffold": "studio_imaginus", "subtext_hint": "SDXL — Detailed Explicit Generation"},
            "illustrious": {"archetype": "a blushing anime enchantress conjuring vibrant hentai illustrations, ecchi manga panels orbiting in a whirlwind of colour", "scaffold": "studio_imaginus", "subtext_hint": "Illustrious — Anime NSFW Generation"},
            "sd15": {"archetype": "a versatile pleasure-mage of classic conjuration, equally at home with softcore tease and hardcore fantasy", "scaffold": "studio_imaginus", "subtext_hint": "SD 1.5 — Classic Explicit Generation"},
            "pony": {"archetype": "a playful illustrator-witch of stylized erotic art, paint and ink swirling into provocative compositions", "scaffold": "studio_imaginus", "subtext_hint": "Pony — Stylized NSFW Art Generation"},
        },
        "BG_STYLES_NSFW": {
            "tavern": "interior of a decadent magical pleasure guild, silk curtains and velvet chaises, warm amber candlelight, scattered enchanted wine goblets, arcane aphrodisiac potions on shelves, intimate alcoves with sheer draping, rose petals floating in enchanted air",
            "library": "forbidden section of an arcane library, towering shelves of erotic grimoires and tantric spell-scrolls, warm reading nooks with plush fur throws, enchanted illustrations that move and blush, soft moaning echoing from deeper stacks, dust motes in amber light",
...
def push_to_nsfw_repo(staging_dir):
    ...
    """
    nsfw_clone = Path("/tmp/nsfw_push_tmp")
    _token_file = HERE / ".github_token"
    _token = _token_file.read_text().strip() if _token_file.exists() else ""
    if _token:
        repo_url = f"https://x-access-token:{_token}@github.com/{NSFW_REPO}.git"
    else:
    ...
    for item in nsfw_clone.iterdir():
        ...
        if item.is_dir():
            ...
            if item.exists():
                subprocess.run(["cmd", "/c", "rd", "/s", "/q", str(item)],
                               check=False, timeout=30, capture_output=True)
        else:
            try:
                item.unlink()
            except PermissionError:
                os.chmod(str(item), _stat.S_IWRITE)
                item.unlink()
    ...
    _git_env = {**os.environ,
                "GIT_COMMITTER_NAME": "64v",
                "GIT_COMMITTER_EMAIL": "laboratoiresonore@users.noreply.github.com",
                "GIT_AUTHOR_NAME": "64v",
                "GIT_AUTHOR_EMAIL": "laboratoiresonore@users.noreply.github.com"}
    subprocess.run(["git", "add", "-A"], cwd=str(nsfw_clone), env=_git_env)
    result = subprocess.run(
        ["git", "commit", "-m",
         "NSFW build: auto-patched from public repo with all NSFW content"],
        cwd=str(nsfw_clone), capture_output=True, text=True, env=_git_env)
    if result.returncode != 0:
...
def build_exe(staging_dir):
    ...
    # Wizard Guild NSFW
    tavern_dir = staging_dir / "tavern"
    guild_launcher = tavern_dir / "wizard_guild_launcher.py"
    if guild_launcher.exists():
        print("\nBuilding Wizard_Guild_NSFW.exe...")
        guild_icon = staging_dir / "assets" / "wizard_guild.ico"
        cmd3 = [
            sys.executable, "-m", "PyInstaller",
            "--noconfirm",
            "--onefile",
            "--console",
            "--name", "Wizard_Guild_NSFW",
            "--add-data", f"{tavern_dir}{sep}tavern",
            "--add-data", f"{staging_dir / 'scaffold'}{sep}scaffold",
            "--distpath", str(staging_dir / "dist"),
            "--workpath", str(staging_dir / "build" / "wizard_guild"),
            str(guild_launcher),
        ]
        if guild_icon.exists():
            cmd3 += ["--icon", str(guild_icon)]

        result3 = subprocess.run(cmd3, cwd=str(staging_dir))
        if result3.returncode == 0:
            src3 = staging_dir / "dist" / "Wizard_Guild_NSFW.exe"
            if src3.exists():
                shutil.move(str(src3), str(NSFW_DIST / "Wizard_Guild_NSFW.exe"))
                print(f"  Built: {NSFW_DIST / 'Wizard_Guild_NSFW.exe'}")
        else:
            print(f"  Wizard Guild NSFW build FAILED (exit {result3.returncode})")
    else:
        print("  WARNING: wizard_guild_launcher.py not found in staging, skipping Guild exe")
...
def _create_staging():
    ...
    Copies the entire repo (minus .git, build, dist, __pycache__, nsfw/)
    into nsfw/staging/ for deterministic patching.
    """
    staging = NSFW_DIR / "staging"
    repo_root = NSFW_DIR.parent

    if staging.exists():
        # Windows locks directories (.github/workflows, indexer, antivirus).
        # Multi-strategy cleanup: clear read-only attrs, retry, rd /s /q, rename.
        import time, subprocess as _sp, stat

        def _force_remove_readonly(func, path, exc_info):
            """onerror/onexc handler: strip read-only and retry."""
            try:
        ...
        # Still alive? Nuke it with rd /s /q on Windows
        if staging.exists() and platform.system() == "Windows":
            # Strip attribs first, then rd
            _sp.run(["cmd", "/c", "attrib", "-r", "-h", "-s",
                      str(staging) + "\\*.*", "/s", "/d"],
                     check=False, timeout=30, capture_output=True)
            _sp.run(["cmd", "/c", "rd", "/s", "/q", str(staging)],
                     check=False, timeout=30, capture_output=True)

        # Last resort: rename the old staging aside so copytree can proceed
        if staging.exists():
            dead = staging.parent / f"staging_old_{int(time.time())}"
            try:
                staging.rename(dead)
                print(f"  WARNING: Could not delete staging, renamed to {dead.name}")
            except OSError:
                print(f"  ERROR: Cannot remove {staging}. Close all apps and delete it manually.")
                sys.exit(1)

    SKIP = {".git", "build", "dist", "__pycache__", "nsfw", "test_st_outputs",
            "tavern/creations", ".mypy_cache", ".pytest_cache", "node_modules"}

    def _ignore(directory, contents):
        rel = os.path.relpath(directory, repo_root)
        ignored = set()
        for c in contents:
            full_rel = os.path.join(rel, c).replace("\\", "/")
            if c in SKIP or full_rel in SKIP:
                ignored.add(c)
            # Skip large binary artifacts
            if c.endswith(".exe") or c.endswith(".dmg"):
                ignored.add(c)
        return ignored

    shutil.copytree(str(repo_root), str(staging), ignore=_ignore)
    print(f"  Staging directory created at {staging}")
    return staging


def _ensure_spellcaster_core(staging):
    """Ensure spellcaster_core is bundled in the GIMP plugin directory.

    The shim files (_nodes.py, _architectures.py, _composites.py, _workflows_v2.py)
    import from spellcaster_core/. This must be present as a sibling directory
    for the plugin to work in standalone installs (not just dev checkouts).
    """
...
def _apply_all_patches(staging):
    """Apply every NSFW patch to the staging directory."""
    _ensure_spellcaster_core(staging)
    patch_manifest(staging)
    patch_gimp_plugin(staging)
    patch_darktable_plugin(staging)
    patch_klein_nsfw(staging)
    patch_version(staging)
    patch_nsfw_assets(staging)
    patch_nsfw_theme(staging)
    patch_auth_auto_update(staging)
    patch_guild_nsfw(staging)
    print("\nAll patches applied.")


def validate_patches(staging):
    """Quick sanity check that critical patches are present."""
    gimp = (staging / "plugins" / "gimp" / "comfyui-connector" / "comfyui-connector.py").read_text(encoding="utf-8")
    dt = (staging / "plugins" / "darktable" / "comfyui_connector.lua").read_text(encoding="utf-8")
    manifest = (staging / "installer" / "manifest.json").read_text(encoding="utf-8")

    checks = [
        ("NSFW LoRAs in manifest", "nsfw" in manifest.lower() or "NSFW" in manifest),
        ("NSFW presets in GIMP", "NSFW" in gimp or "nsfw" in gimp),
        ("NSFW presets in Darktable", "NSFW" in dt or "nsfw" in dt),
    ]
    ok = True
...
if __name__ == "__main__":
    import argparse
    ap = argparse.ArgumentParser(description="Build NSFW edition of Spellcaster")
    ap.add_argument("--patch-only", action="store_true", help="Stage + patch only (no exe or push)")
    ap.add_argument("--build-exe", action="store_true", help="Build exe from existing staging")
    ap.add_argument("--push", action="store_true", help="Push existing staging to NSFW repo")
    args = ap.parse_args()
    ...
    if args.build_exe:
        if not staging.exists():
            print("ERROR: No staging directory. Run without --build-exe first.")
            sys.exit(1)
        build_exe(staging)
    elif args.push:
        if not staging.exists():
            print("ERROR: No staging directory. Run --patch-only first.")
            sys.exit(1)
        push_to_nsfw_repo(staging)
    else:
        staging = _create_staging()
        print("\nApplying NSFW patches...")
        _apply_all_patches(staging)
        validate_patches(staging)

        if not args.patch_only:
            print("\nBuilding executables...")
            build_exe(staging)
            print("\nPushing to NSFW repo...")
            push_to_nsfw_repo(staging)
