...
async function generateMissingAvatars() {
    ...
    document.querySelector('#loading-overlay p').textContent =
        `Generating avatars for ${missing.length} new wizard(s)...`;

    for (let i = 0; i < missing.length; i++) {
        let char = missing[i];
        document.querySelector('#loading-overlay p').textContent =
            `Generating avatar ${i+1}/${missing.length}: ${char.name}...`;
        try {
            const avatarRes = await fetch('/api/avatar_generate', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    // Dismiss spinner — static images are done
    overlay.classList.add('hidden');
    document.querySelector('#loading-overlay p').textContent = "The Guild is thinking...";

    // Queue animated avatars in background (non-blocking, no spinner)
    queueAnimatedAvatars();
}
...
function applyGlobalBackground() {
    ...
    if (!bgUrl) {
        // Fall back to server-side generated assets
